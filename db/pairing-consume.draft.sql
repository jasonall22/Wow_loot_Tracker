-- DRAFT, NOT APPLIED. Atomic pairing consumption for the server-only client.
-- This is SECURITY INVOKER: only service_role receives EXECUTE, and the caller
-- must perform fresh user membership authorization before invoking it.
begin;
...
begin
  ...
  update public.apoc_pairing_challenges
     set consumed_at = now(), device_id = new_device
   where guild_id = challenge.guild_id and id = challenge.id;
