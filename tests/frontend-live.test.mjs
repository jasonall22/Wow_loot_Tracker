...
test('an open raid detail refreshes a removed drop without a user click', async () => {
  ...
  new Script("renderDropRow(sampleLoot, { item_name: 'For Player', winner: 'Player-Realm', award_type: 'OS', dropped_at: new Date(Date.now() - 30 * 60000).toISOString() }); renderDropRow(sampleLoot, { item_name: 'Dust', award_type: 'DE' }); renderDropRow(sampleLoot, { item_name: 'Banked', award_type: 'GB' });").runInContext(context);
  ...
  assert.match(context.sampleLoot.children[0].children[0].children[0].children[1].textContent, /Trade: 1h 30m left/);
  ...
  context.groupedTrash = new Element();
  new Script("renderDropList(groupedTrash, [{ item_name: 'Normal trash', boss: 'Trash' }, { item_name: 'Shard', boss: 'Trash', award_type: 'DE' }, { item_name: 'Bank item', boss: 'Trash', award_type: 'GB' }]);").runInContext(context);
  assert.equal(context.groupedTrash.children.length, 2);
  assert.equal(context.groupedTrash.children[0].children[0].children[0].textContent, 'Trash · Guild / DE');
  assert.equal(context.groupedTrash.children[0].children[1].hidden, true);
  assert.equal(context.groupedTrash.children[1].children[0].children[0].textContent, 'Trash');
  context.gemGroups = new Element();
  new Script("renderDropList(gemGroups, [{ item_name: 'Empyrean Sapphire', boss: 'Dungeon Loot', award_type: 'DE' }, { item_name: 'Pattern: Swiftstrike Shoulders', boss: 'Dungeon Loot', award_type: 'MS' }]);").runInContext(context);
  assert.equal(context.gemGroups.children.length, 2);
  assert.equal(context.gemGroups.children[0].children[0].children[0].textContent, 'Trash');
  assert.equal(context.gemGroups.children[1].children[0].children[0].textContent, 'DE / Guild Bank');
  assert.equal(context.gemGroups.children[1].children[1].hidden, true);
  context.orderedGroups = new Element();
  new Script("renderDropList(orderedGroups, [{ item_name: 'Second kill', boss: 'Second' }, { item_name: 'Trash item', boss: 'Trash' }, { item_name: 'First kill', boss: 'First' }, { item_name: 'Gem', boss: 'Dungeon Loot' }]);").runInContext(context);
  assert.deepEqual(context.orderedGroups.children.map((group) => group.children[0].children[0].textContent), ['Second', 'First', 'Trash', 'DE / Guild Bank']);
